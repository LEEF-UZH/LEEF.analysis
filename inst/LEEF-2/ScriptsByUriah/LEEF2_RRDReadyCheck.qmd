...
options(RRDarrow = "/Volumes/RRD.Reclassification_LEEF-2/parquet_v2.3.8-LEEF-2_20230825/")
...
RMK: The timestamps were not processed due to wrong formated yaml files. The errors are fixed, and the pipeline is processing thse at the moment.

## Check 1.1: densities/biomasses

```{r}
# timestamps and days in RRD
meas <- densities |>
  select(day, timestamp, measurement) |>
  distinct()
days <- unique(meas$day) 
timestamps <- unique(meas$timestamps) 
...
idx_bemov_16 <- which(!(days2 %in% days))
idx <- which(!(days2 %in% days))
idx <- which(!(days2 %in% days))
idx <- which(!(days2 %in% days))
idx <- which(!(days2 %in% days))
...
```{r}
densities |>
  filter(timestamp %in% as.numeric(missing_timestamps)) 
```
## Check 1.2: oxygen

RMK: `20230517` is missing from the raw data. 
```{r}
# timestamps and days in RRD
