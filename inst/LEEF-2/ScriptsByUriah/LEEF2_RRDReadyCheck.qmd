...
format:
  html:
    ...
    toc-depth: 6
params:
  RRDarrow: "/Volumes/UDworkMobil/LEEF2RRD_Creation/parquet_v2.3.8-LEEF-2_20240314"
---
...
TODO [ ]: These are not analysed yet due to machine failure.
...
print("Is Didinium present?")
"Didinium" %in% unique(densities$species)
```
...
print("The following classes are present but shouldn't be:")
classes[!idx]
densities |>
    filter(species %in% classes[!idx]) |>
    select(species, measurement, timestamp) |>
    distinct()
```
...
print("Loxocephalus has been correctly removed from the flowcam")
!Loxo
```
...
print("The following bottles are present but shouldn't be:")
bots[!idx]
densities |>
...
```{r}
NAs_wc <- water_chem %>%
    group_by(type, bottle, timestamp) %>%
...
videos_to_remove_16 <- tocheck_3rdmethod16 %>%
    ...
    mutate(
        video.nr = as.numeric(sub(".+_(.+)", "\\1", file)),
...
videos_to_remove_25 <- full_join(
    videos_to_remove_25_1,
...
compare_densities <- left_join(
    new_densities %>% dplyr::filter(species != "Loxocephallus", species != "Euplotes"),
    densities
)

print("Do the densities in the RRD correspond to the densities in new_densities.csv")
all(round(compare_densities$density, 3) == round(compare_densities$new_densities, 3))
...
morph16 <- arrow_read_table("bemovi_16_morph") %>%
    collect() %>%
    dplyr::filter(
        bottle %in% c("b_02", "b_15", "b_20", "b_31"),
        timestamp >= 20230403,
        species == "Colpidium"
    )
...
compare_densities2 <- left_join(
    new_PCaudatum_densities %>% dplyr::filter(species != "Loxocephallus", species != "Euplotes"),
    densities
)

print("Do the densities in the RRD correspond to the densities in new_PCaudatum_densities.csv")
all(round(compare_densities2$density, 3) == round(compare_densities2$new_densities, 3))
...
compare_densities3 <- left_join(
    new_biomass20221107,
    densities,
    join_by(timestamp, day, bottle, species, density, measurement, incubator, temperature, resources, salinity, replicate)
)
...
compare_densities3 |>
    mutate(identical = round(biomass, 3) == round(new_biomass, 3)) |>
    filter(!identical) |>
    select(c(1:6, 13:15))
```
...
toc <- toc %>%
    dplyr::mutate(experiment_phase = case_when(
...
# Check add missing data points
## flowcam 20230719 b_01
```{r}
#| eval: true
#|

cat("\n\nWrong number of datapoints:\n")
arrow_read_table(table = "flowcam_density") |>
    group_by(timestamp, bottle) |>
    summarise(n = n()) |>
    filter(n != 11) |>
    collect() |>
    knitr::kable()

ts <- arrow_read_table(table = "flowcam_density") |>
    select(timestamp) |>
    distinct() |>
    collect() |>
    unlist() |>
...
cat("\n\nMissing timestamps:\n")
timestamps2[!(timestamps2 %in% ts)]
```

## bemovi_mag_16 20230602 20230616 20230621
```{r}
#| eval: true
#|

cat("\n\nWrong number of datapoints:\n")
arrow_read_table(table = "bemovi_16_density") |>
    group_by(timestamp, bottle) |>
    summarise(n = n()) |>
    filter(n != 6) |>
    collect() |>
...
ts <- arrow_read_table(table = "bemovi_16_density") |>
    select(timestamp) |>
    distinct() |>
    collect() |>
    unlist() |>
...
## bemovi_mag_25_cropped 20230602 20230616 20230621
```{r}
#| eval: true
#|

cat("\n\nWrong number of datapoints:\n")
arrow_read_table(table = "bemovi_25_density_cropped") |>
    group_by(timestamp, bottle) |>
    summarise(n = n()) |>
    filter(n != 1) |>
    collect() |>
...
ts <- arrow_read_table(table = "bemovi_25_density_cropped") |>
    select(timestamp) |>
    distinct() |>
    collect() |>
    unlist() |>
...
## bemovi_mag_25 20230602 20230616 20230621
```{r}
#| eval: true
#|

cat("\n\nWrong number of datapoints:\n")
arrow_read_table(table = "bemovi_25_density") |>
    group_by(timestamp, bottle) |>
    summarise(n = n()) |>
    filter(n != 6) |>
    collect() |>
...
ts <- arrow_read_table(table = "bemovi_25_density") |>
    select(timestamp) |>
    distinct() |>
    collect() |>
    unlist() |>
...
## manualcount for all not measured data points
```{r}
#| eval: true
#|

cat("\n\nWrong number of datapoints:\n")
arrow_read_table(table = "manualcount") |>
    group_by(timestamp, bottle) |>
    summarise(n = n()) |>
    filter(n != 1) |>
    collect() |>
...
ts <- arrow_read_table(table = "manualcount") |>
    select(timestamp) |>
    distinct() |>
    collect() |>
    unlist() |>
...
cat("\n\nWrong number of datapoints:\n")
arrow_read_table(table = "conductivity") |>
    group_by(timestamp, bottle) |>
    summarise(n = n()) |>
    filter(n != 1) |>
    collect() |>
...
ts <- arrow_read_table(table = "conductivity") |>
    select(timestamp) |>
    distinct() |>
    collect() |>
    unlist() |>
...
## O2 **missing (n < 2) or to many** (n > 2) datapoints
...
arrow_read_o2() |>
    group_by(timestamp, bottle) |>
    summarise(n = n(), sensors = paste(sensor |> sort(), collapse = ", ")) |>
    filter(n != 2) |>
    knitr::kable()
...
arrow_read_toc() |>
    group_by(timestamp, bottle) |>
    summarise(n = n(), type = paste(type |> sort(), collapse = ", ")) |>
    filter(n != 4) |>
    knitr::kable()
...
# Check 
TODO Check that salinity etc has been set everywhere, particularly where density is 0!
