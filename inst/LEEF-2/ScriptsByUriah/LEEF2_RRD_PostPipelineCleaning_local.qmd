...
# options(RRDarrow = "/Users/uriah/Documents/1_Projects/LEEF_RRDs/LEEF2/parquet_v2.3.7-LEEF-2_20230813/")
...
tocheck_2ndmethod_MissingTimestamps <- read_csv("VideoChecks/SecondMethodCheck/tocheck_2ndmethod_MissingTimestamps.csv")  %>%
  mutate(faulty_video = ifelse(decision=="ok",F,T),
         faulty_video_int = ifelse(decision=="ok",0,1)) 

tocheck_2ndmethod <- rbind(tocheck_2ndmethod, tocheck_2ndmethod_MissingTimestamps)
...
CalculateDensities <- function(morph, meas, extrapolation.factor, cropping.factor) {
  density <- morph %>%
    ...
    mutate(nfiles = length(unique(file))) %>%
    group_by(timestamp, bottle, species, nfiles) %>%
    summarise(
      density = sum(dens.ml) / (unique(nfiles) * 125)
    )%>%
...
# test <- morph25 %>%
#   dplyr::filter(timestamp==20230512)
...
# test <- morph25 %>%
#   dplyr::filter(timestamp==20221209)
...
# old_densities <- old_densities[!is.na(old_densities$day), ] # note 201023: this step is only necessary becuase colpidium is missing on some days? that it shouldnt?
...
bac_sum <- densities %>%
  dplyr::filter(species=="bacteria") %>%
  group_by(timestamp, day, bottle, replicate, temperature, resources, salinity) %>%
  summarize(MedianDensity = median(density),
...
bac_sum %>%
  ...
  geom_text(data=annot, aes(x = -Inf, y = -Inf, label = bottle),
            hjust = -0.5, vjust = -7,col="black")   +
  labs(title = "Median density") +
  
bac_sum %>%
  ggplot(aes(day, MeanDensity))+
  geom_line() +
  facet_grid(conditions ~ replicate, scales = "free")+
  geom_point(data = bac_sum %>% dplyr::filter(timestamp %in% c(20230407, 20230531, 20230602)),
             shape=21, col="red") +
...
bac %>%
  dplyr::filter(bottle=="b_30") %>%
  ggplot(aes(day, density, col=sample))+
  geom_line() +
  geom_point(data = bac %>% dplyr::filter(timestamp %in% c(20230407, 20230531, 20230602),bottle=="b_30"),
             shape=21, col="black") +
  theme_bw()+
  theme(legend.position = "bottom")+
  labs(title = "Bottle 30") +

bac_sum %>%
  dplyr::filter(bottle=="b_30") %>%
  ggplot(aes(day, MedianDensity))+
  geom_line() +
  geom_point(data = bac_sum %>% dplyr::filter(timestamp %in% c(20230407, 20230531, 20230602),bottle=="b_30"),
             shape=21, col="red") +
  theme_bw()+
  labs(title = "Median density, bottle 30") +
...
bac_sum %>%
  ...
  geom_point(data = bac_sum %>% dplyr::filter(timestamp %in% c(20230407, 20230531, 20230602),bottle=="b_30"),
             shape=21, col="red") +
  theme_bw()+
  labs(title = "Mean density, bottle 30")
```
...
ciliate_biomasses_16 <- ciliate_traits_16 %>%
  group_by(timestamp, bottle, species) %>%
  summarize(biomass = sum(biomass*n_frames, na.rm = T)/(length(unique(file))*125)) %>%
  mutate(biomass = biomass*extrapolation.factor_16*cropping.factor_16,
         measurement = meas_16,
