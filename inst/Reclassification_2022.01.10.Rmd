...
output:
  html_document:
    ...
    code_folding: hide
params:
  output_dir: ~/RRD.Reclasification_XXX/
---
...
```{r}
base <- paste0("https://github.com/LEEF-UZH/LEEF.parameter/raw/", "v1.2.1", "/")
```
...
download.file(
  paste0(base, "parameter/flowcam/svm_flowcam_classifiers_18c_december2021.rds"), 
...
download.file(
  paste0(base, "parameter/flowcam/svm_flowcam_classifiers_increasing_trained_at_18c_december2021.rds"), 
...
class_constant <- file.path(pdir, "class_constant.rds")
download.file(
  paste0(base, "parameter/bemovi.mag.16/svm_video_classifiers_18c_16x_december2021.rds"), 
  destfile = class_constant,
  mode = "wb"
)
...
class_increasing <- file.path(pdir, "class_increasing.rds")
download.file(
  paste0(base, "/parameter/bemovi.mag.16/svm_video_classifiers_increasing_trained_at_18c_16x_december2021.rds"), 
  destfile = class_increasing,
  mode = "wb"
)
```
...
class_constant <- file.path(pdir, "class_constant.rds")
download.file(
  paste0(base, "parameter/bemovi.mag.25/svm_video_classifiers_18c_25x_december2021.rds"), 
  destfile = class_constant,
  mode = "wb"
)
...
class_increasing <- file.path(pdir, "class_increasing.rds")
download.file(
  paste0(base, "parameter/bemovi.mag.25/svm_video_classifiers_increasing_trained_at_18c_25x_december2021.rds"), 
  destfile = class_increasing,
  mode = "wb"
)
```
...
system.time(
  add_reclassified_to_db(
    path = output, 
    db = file.path(output, "LEEF.RRD.sqlite")
  )
)
```

# Overlays
TODO This still needs to be discussed. Options include using `bemovi.LEEF::create_overlays_subtitle_directory()` in a standalone function or `bemovi.LEEF::create_overlays_subtitle_single()` directly in the `classify_bemovi_archive()` function. Likely the first option of=s better as more flexible.
