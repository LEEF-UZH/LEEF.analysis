...
timestamps <- c(20211231, 20220103, 20220105)
```
...
out <- file.path(output, "bemovi_extract.mag.16")
if (!file.exists(out)) {
...
out <- file.path(output, "bemovi_extract.mag.25")
if (!file.exists(out)) {
  system.time(
    LEEF.analysis::classify_bemovi_archive( 
      timestamps = timestamps, 
  ...
  message(
    "Directory ", out, "exists\n",
    "Skipping 'bemovi_extract.mag.25'"
  )
}
```
...
out <- file.path(output, "bemovi_extract.mag.25.non_cropped")
if (!file.exists(out)) {
  system.time(
    LEEF.analysis::classify_bemovi_archive( 
      timestamps = timestamps, 
  ...
  message(
    "Directory ", out, "exists\n",
    "Skipping 'bemovi_extract.mag.25.non_cropped'"
  )
}
```
...
out <- file.path(output, "bemovi_extract.mag.25.cropped")
if (!file.exists(out)) {
  system.time(
    LEEF.analysis::classify_bemovi_archive( 
      timestamps = timestamps, 
  ...
  message(
    "Directory ", out, "exists\n",
    "Skipping 'bemovi_extract.mag.25.cropped'"
  )
}
```
