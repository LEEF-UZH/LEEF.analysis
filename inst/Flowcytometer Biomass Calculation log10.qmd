...
if (!dir.exists(file.path(params$output_dir, "extracted"))){
  dir.create(file.path(params$output_dir, "extracted"), showWarnings = FALSE, recursive = TRUE)
  
  file.copy(
    file.path(params$extracted_dir, paste0("LEEF.fast.flowcytometer.", as.character(params$timestamp))),
    file.path(params$output_dir, "extracted"),
    recursive = TRUE,
    copy.mode = FALSE
  )
  ...
  file.rename(
    ...
    to   = file.path(params$output, "extracted", "flowcytometer")
  )
}
...
datadir <- file.path(params$output_dir, "extracted", "flowcytometer")
...
traits$length_H <- traits$FSC.H * ss_repaired$slope_H + ss_repaired$intercept_H
