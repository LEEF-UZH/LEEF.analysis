...
system.time(
  add_reclassified_to_db(
    ...
    method = c("flowcam", "bemovi_mag_16", "bemovi_mag_25")
  )
)
...
# 5. TODO - Flowcytometer
## TODO - Flowcytometer densities
...
density_flowcytometer_archive <- function(
  ...
  gates_coordinates = read.csv(paste0(base, "parameter/flowcytometer/gates_coordinates.csv")),
  timestamps = timestamps,
...
system.time(
  add_reclassified_to_db(
    ...
    remove_timestamps = timestamps,
    backup_removed = FALSE,
    method = c("flowcytometer")
  )
)

file.rename(db_5_tmp, db_5)
```

## TODO - Trait Extraction Flowcytometer and Volume Calculation
...
# Create Indices and rename species
```{r}
#| label: indices_rename
file.copy(db_5, db_final)

RRD_LEEF_1_create_indices(db = db_final)
RRD_LEEF_1_rename(db = db_final)
