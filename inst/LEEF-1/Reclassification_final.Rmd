...
reclass_bottle_db <- gsub(".sqlite", paste0(".bottle.sqlite"), reclass_db)

bemovi_filter_db <- gsub(".sqlite", paste0(".bemovi.sqlite"), reclass_bottle_db)
...
```{r addRRD}
file.copy(org_db, "tmp.reclass.sqlite")
...
file.rename("tmp.reclass.sqlite", reclass_db)
```
...
```{r}
file.copy(reclass_db, "tmp.bottle.sqlite")
...
file.rename("tmp.bottle.sqlite", reclass_bottle_db)
...
file.copy(reclass_bottle_db, "tmp_filter_bemovi.sqlite")

conn <- DBI::dbConnect(
  drv = RSQLite::SQLite(),
  db = "tmp_filter_bemovi.sqlite"
)
DBI::dbBegin(conn)
...
file.rename("tmp_filter_bemovi.sqlite", bemovi_filter_db)
```
...
```{r label, eval=FALSE}
file.copy(bemovi_filter_db, "tmp_filter_flowcam.sqlite")

conn <- DBI::dbConnect(
  drv = RSQLite::SQLite(),
  db = "tmp_filter_flowcam.sqlite"
)
DBI::dbBegin(conn)
...
file.rename("tmp_filter_flowcam.sqlite", flowcam_filter_db)
```
...
```{r echo = FALSE}
db_vac <- gsub("\\.sqlite$", ".vacuumed.sqlite", flowcam_filter_db)
cat(paste0("sqlite3 \"./", basename(flowcam_filter_db), "\" 'VACUUM main INTO \"", basename(db_vac), "\"'"))
