...
## 3. [x] Exclusion of particles that have been classified as *Staurastrum gracile* (i.e. Staurastrum1) with a probability of less than 95% (already incorporated in RRD, code not run here and not completely up-to-date. Rainer has the complete up-to-date code)
```{r}
#| label: test_3
#| 
...
for (tbl in tbls){
  x <- dat |>
    filter(species == "Staurastrum1" & probability < 0.95) |>
    select(species, probability) |> 
    distinct() |> 
    collect() |>
    unlist()
  if (length(x) == 0){
...
## 9. [ ] exclusion of all flowcytometer particles where FSC.A < 2 (raw FSC.A < 100)
## 10. [ ] Fix EAWAG daten (Hallo Rainer, mir ist aufgefallen, dass in der LEEF1-RRD bei den flowcytometer-Daten, die an der Eawag gemessen wurden, etwas nicht stimmt. z.B. bei timestamp "20220413" steht bei date "2022-05-17" und es gibt 3 Replikate (ich habe aber nur zwei gemessen))
