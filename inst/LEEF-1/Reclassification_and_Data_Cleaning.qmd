...
pbapply::pblapply(
    ...
    function(fn_traits) {
      cat(basename(fn_traits))
        ...
        density <- rbind(density, densityStau1)

        unlink(fn_traits)
        unlink(fn_density)

        saveRDS(traits, file = fn_traits)
        saveRDS(densityStau1, file = fn_density)
    }
)
```
...
```{r}
#| label: species_to_0_bemovi_25_morph_mvt_2
#| 
...
densityStau1 <- stau1traits %>%
  ...
  summarize(count=n(), biomass = sum(biomass)) %>%
  mutate(density = count * (dilution_factor / volume_imaged))
...
# 3. Add O2, Manualcount and TOC measuremnts 
...
# final. Create final RRD database
```{r}
