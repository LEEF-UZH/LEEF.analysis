...
## Reclassification of Bemovi magnification 16
### Prepare Bemovi 16
```{r prepare_bemovi_16}
pdir <- tempfile()
dir.create(pdir, showWarnings = FALSE, recursive = TRUE)
...
## Reclassification of Bemovi magnification 25
### Prepare Bemovi 25
```{r prepare_bemovi_25}
pdir <- tempfile()
dir.create(pdir, showWarnings = FALSE, recursive = TRUE)
...
## Flowcytometer select only needed species in densities
...
## bemovi - Exclusion of time series that are wrong classifications or only noise
```{r}
...
## flowcam - Adding density=0 for some special cases
