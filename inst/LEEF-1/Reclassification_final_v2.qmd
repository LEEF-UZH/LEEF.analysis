...
download.file(
  paste0(base, "parameter/flowcytometer/conv_FSCA_length"), 
...
extract_traits_flowcytometer_archive(
  ...
  length_slope = conv_FSCA_length$slope,
  length_intercept = conv_FSCA_length$intercept,
  mc.cores = params$cores
)
```
...
unlink(pdir)
rm(gates_coordinates, metadata_flowcytometer, intercept)
```
...
RRD_create_indices(db_name(1, temp = TRUE))
...
options(RRDdb = db_name(3, temp = FALSE))
...
# 5. TODO Add O2, Manualcount and TOC measuremnts and experimental tables
## TODO Prepare Adding
```{r prepare_adding}
pdir <- tempfile()
dir.create(pdir, showWarnings = FALSE, recursive = TRUE)
...
# 6. VERIFY Rename species and columns
```{r}
#| label: indices_rename
file.copy(db_name(5, temp = FALSE), db_name(6, temp = TRUE))

RRD_LEEF_1_rename(db = db_name(6, temp = TRUE))

file.rename(db_name(6, temp = TRUE), db_name(6, temp = FALSE))

file.copy(db_name(6, temp = FALSE), db_name("final", temp = FALSE))
```
