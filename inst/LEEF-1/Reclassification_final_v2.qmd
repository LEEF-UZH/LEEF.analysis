...
input:
  html_document:
    ...
    code_folding: hide  
params:
  output_dir: "/Volumes/Documents/RRD.Reclassification_final/"
  version_tag: "v1.8.1"
  base: "https://github.com/LEEF-UZH/LEEF.parameter/raw/"
  db: "/Volumes/Documents/RRD.Reclassification_final/LEEF.RRD.sqlite"
...
db_org <- db
db_1 <- gsub(".sqlite", paste0(".", params$version_tag, "_1.sqlite"), db_org)
db_1_tmp <- paste0(db_1, ".tmp")
db_2 <- gsub(".sqlite", paste0(".", params$version_tag, "_2.sqlite"), db_org)
db_2_tmp <- paste0(db_2, ".tmp")
db_3 <- gsub(".sqlite", paste0(".", params$version_tag, "_3.sqlite"), db_org)
db_3_tmp <- paste0(db_3, ".tmp")
db_4 <- gsub(".sqlite", paste0(".", params$version_tag, "_4.sqlite"), db_org)
db_4_tmp <- paste0(db_4, ".tmp")
db_5 <- gsub(".sqlite", paste0(".", params$version_tag, "_5.sqlite"), db_org)
db_5_tmp <- paste0(db_5, ".tmp")
db_6 <- gsub(".sqlite", paste0(".", params$version_tag, "_6.sqlite"), db_org)
db_6_tmp <- paste0(db_6, ".tmp")
db_7 <- gsub(".sqlite", paste0(".", params$version_tag, "_7.sqlite"), db_org)
db_7_tmp <- paste0(db_7, ".tmp")

db_final <- gsub(".sqlite", paste0(".", params$version_tag, ".final.sqlite"), db_org)
...
if (file.exists(db_org)) {
  stop(db_org, " exists and will not be overwritten!\n", "If you want to re-generate, please rename the database or delete it!")
}
RRD_new(db_org, LEEF = "LEEF-1")
```

# 1. DONE - Re-calculations for all timestamps
## DONE - Reclassification of Flowcam
...
## DONE - Reclassification of Bemovi magnification 16
### Prepare Bemovi 16
```{r prepare_bemovi_16}
pdir <- tempfile()
dir.create(pdir, showWarnings = FALSE, recursive = TRUE)
...
## DONE - Reclassification of Bemovi magnification 25
### Prepare Bemovi 25
```{r prepare_bemovi_25}
pdir <- tempfile()
dir.create(pdir, showWarnings = FALSE, recursive = TRUE)
...
system.time(
  add_reclassified_to_db(
    ...
    methods = c("flowcam", "bemovi_mag_16", "bemovi_mag_25")
  )
)

RRD_create_indices(db_1_tmp)

file.rename(db_1_tmp, db_1)
```




# 2. DONE - Re-calculations of individual bottles and timestamps
## Flowcam Bottle `b_02`
...
### Set output path
```{r output_path_b}
out <- file.path(params$output_dir, paste0("flowcam_", bott, "_", format(after, scientific = FALSE), "_", format(until, scientific = FALSE)))
```
...
file.rename(db_2_tmp, db_2)

```

# 3. DONE - Set species to 0 which were mis-classified
...
file.rename(db_3_tmp, db_3)
```


# 4. DONE - Flowcam Stau1
...
file.rename(db_4_tmp, db_final)
```

# 5. VERFIFY - Flowcytometer
...
size_standard <- file.path(pdir, "size_standard.csv")
download.file(
  paste0(base, "parameter/flowcytometer/size_standard.csv"), 
  destfile = size_standard,
  mode = "wb"
)
```

## VERIFY Do It
### VERIFY - Flowcytometer densities
...
### VERIFY - Trait Extraction Flowcytometer and Volume Calculation
...
### VERIFY Add densities to database
```{r add_flowcytometer}
file.copy(db_4, db_5_tmp)
...
## VERIFY Clean Up Flowcytometer
```{r cleanup_flowcytometer}
unlink(pdir)
rm(gates_coordinates, metadata_flowcytometer, size_standard)
```

# 6. TODO Add O2, Manualcount and TOC measuremnts and experimental tables
## TODO Prepare Adding
```{r prepare_adding}
pdir <- tempfile()
dir.create(pdir, showWarnings = FALSE, recursive = TRUE)
...
TODO: 
  - Add composition
  - Add immigration_schedule
  - Add experimental_design
  - Add light_decline_schedule
  
file.rename(db_6_tmp, db_6)
```


## VERIFY Clean Up Adding
```{r cleanup_flowcytometer}
unlink(pdir)
rm(gates_coordinates, metadata_flowcytometer, size_standard)
```

# 7. VERIFY Rename species and columns
```{r}
#| label: indices_rename
file.copy(db_5, db_7_tmp)

RRD_LEEF_1_rename(db = db_7_tmp)

file.rename(db_7_tmp, db_7)
file.copy(db_5, db_7_tmp)
```
...
```{r echo = FALSE}
db_vac <- gsub("\\.sqlite$", ".vacuumed.sqlite", db_final)
cat(paste0("sqlite3 \"./", basename(db_final), "\" 'VACUUM main INTO \"", basename(db_final), "\"'"))
