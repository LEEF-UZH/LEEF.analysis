...
## TODO - Exclusion of time series that are wrong classifications or only noise
```{r}
#| 
excl <- data.frame(
  bottles = c(
  ...
  species = c(
    "Tetrahymena",   "Tetrahymena",   "Tetrahymena",   "Euplotes",
...
## TODO - Adding density=0 for some special cases
...
fns <- list.dirs(path = params$output_dir, recursive = FALSE) |> 
  ...
  grep(pattern = "density", value = TRUE)
...
sapply(
  ...
  function(fn){
    ...
    {
      # and =here goes the stuff fo the rest
    }
    saveRDS(x, fn)
  }
)
