...
if (!file.exists(out)) {
  system.time(
    classify_bemovi_archive( 
      timestamps = timestamps[!(excl %in% timestamps)], 
      archive_dir = params$archive_dir, 
      magnification = 25, 
      classifier_constant_name = class_constant, 
      classifier_increasing_name = class_increasing, 
...
### DONE - Trait Extraction Flowcytometer and Volume Calculation
...
excl <- data.frame(
  ...
  species = c("Tetrahymena",   "Tetrahymena",   "Tetrahymena",   "Euplotes",
              "Euplotes",      "Loxocephallus", "Euplotes",      "Colpidium",
              "Loxocephallus", "Loxocephallus", "Dexiostoma",    "Dexiostoma",
              "Euplotes",      "Euplotes",      "Colpidium",     "Colpidium",
              "Loxocephallus", "Loxocephallus", "Tetrahymena",   "Tetrahymena",
              "Loxocephallus", "Loxocephallus", "Tetrahymena",   "Tetrahymena",  
              "Colpidium",     "Colpidium",     "Dexiostoma",    "Dexiostoma", 
              "Stylonychia2",  "Euplotes",      "Euplotes",      "Staurastrum1",
              "Tetrahymena",   "Tetrahymena",   "Colpidium",     "Colpidium",   
              "Loxocephallus", "Loxocephallus", "Stylonychia2",  "Euplotes",   
              "Euplotes",      "Colpidium",     "Colpidium",     "Loxocephallus",
              "Loxocephallus", "Dexiostoma",    "Dexiostoma",    "Euplotes",    
              "Euplotes",      "Tetrahymena",   "Tetrahymena",   "Colpidium",   
              "Colpidium",     "Loxocephallus", "Loxocephallus", "Euplotes",   
              "Euplotes",      "Staurastrum1",  "Colpidium",     "Colpidium",   
              "Loxocephallus", "Loxocephallus", "Dexiostoma",    "Dexiostoma",  
              "Euplotes",      "Euplotes",      "Colpidium",     "Colpidium",  
              "Loxocephallus", "Loxocephallus", "Dexiostoma",    "Dexiostoma",  
              "Stylonychia2",  "Euplotes",      "Euplotes",      "Staurastrum2", 
              "Staurastrum2",  "Staurastrum2",  "Staurastrum2",  "Staurastrum2", 
              "Staurastrum2",  "Tetrahymena",   "Stylonychia2",  "Stylonychia2") 
)
...
## TODO - Manual correction of time series
```{r man_correction}

fix_bottles <- c("b_14", "b_25")
fix_species <- c("Euplotes", "Stylonychia2")

fns <- list.dirs(path = params$output_dir, recursive = FALSE) |> 
  grep(
    pattern = "bemovi", 
    value = TRUE
  ) |>
  ...
  grep(pattern = "density|morph_mvt", value = TRUE)
...
sapply(
  ...
  function(fn){
    ...
    if (grepl("morph_mvt", fn)){
      i <- (x$species == fix_species[1]) & (x$bottle %in% fix_bottles)
      x$species[i] <- fix_species[2] 
      x$Euplotes_prob <- NA
    } else if (grepl("mean_density", fn)) {
      for (i in 1:length(fix_bottles)){
        wci <- (x$species == fix_species[1]) & (x$bottle == fix_bottles[[i]])
        if (any(wci)) {
          cti <- (x$species == fix_species[2]) & (x$bottle == fix_bottles[[i]])
          x$density[wci] <- sum(x$density[c(wci, cti)])
          x <- x[-wci,]
        }
      }
      saveRDS(x, fn)
    } else {
      stop("Serious error - non conform file name!!!!\n", "Check everything!!!!")
    }
  }
)
...
## DONE - Add to database and create indices
...
# 2. DONE - Set species to 0 which were mis-classified
...
exp_design <- db_read_table(table = "experimental_design") %>% dplyr::collect()
...
morph25_density <- CalculateDensities(
  morph25, 
  meas
)
morph25_density <- SetNotFoundSpeciesTo0(
  morph25_density, 
  moving_background, 
  meas, 
  exp_design, 
...
options(RRDdb = db_name(2, temp = FALSE))
...
# 4. TODO Add O2, Manualcount and TOC measuremnts 
...
toc <- readRDS(params$toc) %>%
  left_join(db_read_table(db = db_name(4, temp = TRUE), table = "experimental_design") %>% collect()) %>%
  filter(!is.na(conc))
...
# 5. VERIFY Rename species and columns
```{r}
#| label: indices_rename
file.copy(db_name(4, temp = FALSE), db_name(5, temp = TRUE))

RRD_LEEF_1_rename(db = db_name(5, temp = TRUE))

file.rename(db_name(5, temp = TRUE), db_name(5, temp = FALSE))

file.copy(db_name(5, temp = FALSE), db_name("final", temp = FALSE))
```
