...
plot_density_species_per_bottle_per_timestamp(params$db, "bemovi_mag_16")
```


### bemovi_mag_25
```{r dens_species_25, fig.height = 16}
plot_density_species_per_bottle_per_timestamp(params$db, "bemovi_mag_25")
```

### bemovi_mag_25_cropped
```{r dens_species_25c, fig.height = 16}
plot_density_species_per_bottle_per_timestamp(params$db, "bemovi_mag_25_cropped")
```

### flowcam
```{r dens_species_fca, fig.height = 16}
plot_density_species_per_bottle_per_timestamp(params$db, "flowcam")
```

### flowcytometer
```{r dens_species_fcy, fig.height = 16}
plot_density_species_per_bottle_per_timestamp(params$db, "flowcytometer")
```

### manualcount
```{r dens_species_mc, fig.height = 16}
plot_density_species_per_bottle_per_timestamp(params$db, "manualcount")
```
...
shiny::renderPlot(
  LEEF.analysis::plot_density_species_per_bottle_per_timestamp(params$db, input$measurement),
  height = 1600
)
```
